...
public class SpringFollowCamera : MonoBehaviour
{
    ...
    private void Start()
    {
		...
		targetCamPos = GameObject.Find("TargetCamPosition").transform;
		SetTargetToPlayer();
	}
    ...
    {
		target = GameObject.Find("PlayerAvatar").transform;
		DidChangeTarget();
	}
	...
	void Apply(Transform dummyTarget, Vector3 dummyCenter)
	{
		...
		if (Input.GetKey(KeyCode.W) && !isSnapping)
		{
	...
	void GoToTargetingPos()
    {
		Vector3 position = transform.position;

		Vector3 newPosition;
		newPosition.x = Mathf.SmoothDamp(position.x, targetCamPos.position.x, ref velocity.x, smoothLag, maxSpeed);
		newPosition.z = Mathf.SmoothDamp(position.z, targetCamPos.position.z, ref velocity.z, smoothLag, maxSpeed);
		newPosition.y = Mathf.SmoothDamp(position.y, targetCamPos.position.y, ref velocity.y, smoothLag, maxSpeed);
